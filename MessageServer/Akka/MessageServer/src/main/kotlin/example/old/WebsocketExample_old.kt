...
public fun handleRequest(request : HttpRequest,system: ActorSystem): HttpResponse {
...
public fun greeter(system: ActorSystem) : Pair<Flow<Message, Message, NotUsed>, TestPublisher.Probe<Message>> {
    ...
    var sink = Sink.foreach<Message> { x -> println("Received 2 " + x.asTextMessage().strictText) }
    var outProbe = TestPublisher.probe<Message>(0,system)
    var flow = Flow.fromSinkAndSource(sink,Source.fromPublisher(outProbe))

    return Pair(flow,outProbe)
}
